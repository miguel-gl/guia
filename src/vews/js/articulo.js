...
function get_articulo(id,id_bd)
{
    if(id==0)
    {
        get_articulo_bd(id_bd);
    }
    else
    {
        $.getJSON("bd/alumnos/articulos.json", function(datos) 
        {
            ...
            var curioso=json['dato_interesante'];
            if(curioso!=null)
            {

                curioso=quitar_estilos(curioso);
                $('#curioso').show();
                $('#curioso').html('<h3 class="text-blue">Dato Interesante</h3>'+curioso);
            }
            var video=json['video'] 
            if(video!=null)
            {
                console.log('si video en bd: '+video)
                $('#Recursos').show();
                $('#videoOk').show();
                document.getElementById('video').innerHTML = '<a data-fancybox href="' + video + '"><img class="recursos-img hvr-pop" src="images/iconos/multimedia.png" alt="Video"></a>';
            }
...
function get_articulo_bd(id_bd)
{
    ...
    $.getJSON("bd/alumnos/articulos.json", function(datos) 
    {
        ...
        json.forEach(j => 
        {
            ...
            if (id_bd==id)
            {
                ...
                var curioso=j.dato_interesante;
                if(curioso!=null)
                {
                    curioso=quitar_estilos(curioso);
                    $('#curioso').show();
...
$('#que_aprendo').html('');
    ...
    let parrafos = [];
    let imgs = [];
    $.getJSON("bd/alumnos/alumnos_parrafos_img.json", function(datos)
    {
        var json=datos[2]["data"];
        //si hay un valor
        var idx=0;
    ...
    $.getJSON("bd/alumnos/alumnos_parrafos.json", function(datos)
    {
        var json=datos[2]["data"];
    ...
    {
        var queaprendo=s[1];
        queaprendo=quitar_estilos(queaprendo);
        $('#que_aprendo').append('<div class="tema-box">'+queaprendo+'</div>');
        ...
        if(path!=null)
        {
            ...
            if(pie!=null)
            {
                $("#que_aprendo").append('<div class="container-fluid"><div class="d-flex flex-row flex-wrap justify-content-center"><div class="container"><div class="row"><div class="col-12"><a data-fancybox="gallery" href="bd/alumnos/'+path+'" class="img-theme" data-caption="'+pie+'"><img src="bd/alumnos/'+path+'" style="width:100%;"><span>'+pie+'</span></a></div></div></div></div></div>');
            }
            else
            {
                $("#que_aprendo").append('<div class="container-fluid"><div class="d-flex flex-row flex-wrap justify-content-center"><div class="container"><div class="row"><div class="col-12"><a data-fancybox="gallery" href="bd/alumnos/'+path+'" class="img-theme" data-caption="imagen'+i+'"><img src="bd/alumnos/'+path+'" style="width:100%;"><span>imagen'+i+'</span></a></div></div></div></div></div>');
            }
        }
        id_array++;
        i++;
...
$(document).ready(function () 
{
    ...
    imp(id_articulo_bd);
    juego( id_articulo_bd );
    quiz( id_articulo_bd  );
    iVideo( id_articulo_bd );
